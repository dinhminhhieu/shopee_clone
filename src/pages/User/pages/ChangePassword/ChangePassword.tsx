...
const ChangePassword = () => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      if (isAxiosUnprocessableEntityError<ErrorResponse<FormDataPassword>>(error)) {
        const formError = error.response?.data.data
        if (formError) {
          Object.keys(formError).forEach((key) => {
            setError(key as keyof FormDataPassword, {
              message: formError[key as keyof FormDataPassword] as string,
              type: 'Server'
            })
          })
        }
  ...
  return (
    <div className='rounded-md bg-white px-2 pb-10 md:ml-14 shadow md:px-7 md:pb-20'>
      ...
      <form onSubmit={onSubmit} className='mt-8 flex flex-col-reverse mr-auto max-w-2xl md:flex-row md:items-start'>
